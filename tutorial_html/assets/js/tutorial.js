/*
Tutorial File: tutorial_html/assets/js/tutorial.js
...
function runHtmlDemo(topic) {
  const output = document.getElementById('preview');
  if (!output) return;
  const now = new Date().toLocaleTimeString();
  output.innerHTML = `<strong>${topic}</strong> demo executed at ${now}.`;
}
...
window.addEventListener('DOMContentLoaded', wireSearch);
